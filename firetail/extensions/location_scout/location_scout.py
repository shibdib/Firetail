...
class LocationScout:
    ...
    async def format_constellation(self, ctx, data):
        async with ctx.channel.typing():
            ...
            current_fights = []
            for fights in sov_battles:
                if fights['constellation_id'] == data['constellation_id'] and 'defender_score' in fights:
                    if fights['defender_score'] == 0.6:
                        continue
                    active_sov = True
                    target_system_id = fights['solar_system_id']
    ...
    async def format_region(self, ctx, data):
        async with ctx.channel.typing():
            ...
            current_fights = []
            for fights in sov_battles:
                if fights['constellation_id'] in data['constellations'] and 'defender_score' in fights:
                    if fights['defender_score'] == 0.6:
                        continue
                    active_sov = True
                    target_system_id = fights['solar_system_id']
